...
## Step 0 — Read handoff and run CodeCop (REQUIRED before any review)

### 0a — Read the walkthrough

Read `.agent/handoffs/current-walkthrough.md`.
This is your input. Do not proceed until you have read it.

If the file does not exist or is empty: return immediately to Team Lead.
> "Walkthrough handoff file not found at `.agent/handoffs/current-walkthrough.md`. Developer must write walkthrough to this path before review can proceed."

Do not ask the user to paste the walkthrough manually.

### 0b — CodeCop full-diff scan

Run `/inboxer-codecop` across all files listed in the walkthrough's `git diff main --name-only` output.

```
CODECOP PRE-SCAN
----------------
Files in diff: [list from walkthrough git diff section]
```

Output the full CodeCop report (all ten rules, PASS/FLAG/N/A for each).

**If CodeCop returns FLAGS FOUND: return FAIL immediately.**
Do not proceed to Step 1. Return to Developer with the CodeCop report.
The Developer must fix all flags, re-run CodeCop, rewrite the handoff file, and notify Team Lead.

**If CodeCop returns CLEAN: proceed to Step 1.**
...
CODECOP SCAN: [CLEAN / FLAGS FOUND — detail]
